...
class DSC(Disaggregator):
    ...
    def learn_dictionary(self, appliance_main, app_name):
        ...
        if app_name not in self.dictionaries:
            ...
            model = MiniBatchDictionaryLearning(n_components=self.n_components, positive_code=True, positive_dict=True,
                                                transform_algorithm='lasso_lars', alpha=self.sparsity_coef)
        ...
        app_name, mean_squared_error(reconstruction, appliance_main) ** (.5)))
    ...
    def partial_fit(self, train_main, train_appliances, **load_kwargs):
        ...
        train_main = pd.concat(train_main,
                               axis=1)  # np.array([i.values.reshape((self.sequence_length,1)) for i in train_main])
        ...
        if len(train_main) > 10:
            ...
            print("--" * 15)
            model = SparseCoder(dictionary=concatenated_bases.T, positive_code=True, transform_algorithm='lasso_lars',
                                transform_alpha=self.sparsity_coef)
            ...
            model = SparseCoder(dictionary=optimal_b.T, positive_code=True, transform_algorithm='lasso_lars',
                                transform_alpha=self.sparsity_coef)
    ...
    def disaggregate_chunk(self, test_main_list):
        ...
        for test_main in test_main_list:
            ...
            for cnt, app_name in enumerate(self.power):
                ...
                predicted_usage = np.matmul(self.reconstruction_bases[:, start_comp:start_comp + n_comps],
