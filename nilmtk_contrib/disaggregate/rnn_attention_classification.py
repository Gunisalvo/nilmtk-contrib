...
class RNN_attention_classification(Disaggregator):
            ...
            if train_main.size > 0:
                ...
                if len(train_main) > 10:
                    ...
                    history = model.fit(train_x, [train_y, appliance_train_classification],
                                        validation_data=(v_x, [v_y, appliance_val_classification]),
                                        epochs=self.n_epochs, callbacks=[checkpoint], batch_size=self.batch_size)
    ...
    def return_network(self):
        ...
        # 1D Conv
        y = Conv1D(16, 4, activation="linear", input_shape=(self.sequence_length, 1), padding="same", strides=1)(
            input_data)
        ...
        full_model = Model(inputs=input_data, outputs=[output, classification_output],
                           name="RNN_attention_classification")
